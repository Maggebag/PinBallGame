...
threepp::Vector3
ballPosGet(const std::shared_ptr<threepp::Mesh> &pinBall, threepp::BulletPhysics &bullet) {
...
threepp::Vector3 getGravFromAngle(float angle) { //input in degrees
    ...
    Gravity.x = 0;
    Gravity.y = -9.81f * std::cos(angleInRad) * 100;
    Gravity.z = 9.81f * std::sin(angleInRad) * 100;
    return Gravity;
}
