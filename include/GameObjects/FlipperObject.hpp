...
public:
    ...
    void addFlipper(float ballSize, threepp::BulletPhysics &bullet, threepp::Object3D &scene) {
        ...
        btFlipper = std::make_unique<btHingeConstraint>(*bulletFlipper->body,
                                                        btVector3(flipperDir_ * ((3.5f * ballSize) / 2.5f), 0, 0),
                                                        btVector3(0, 1, 0));
...
private:
    ...
    static std::shared_ptr<threepp::Mesh>
    createFlipperMesh(int direction, float ballSize) { //-1 for left flipper and 1 for right flipper
        ...
        const auto flipperAxisGeometry = threepp::CylinderGeometry::create(flipperGeometry->height / 2 - 2,
                                                                           flipperGeometry->height / 2 - 2, 27);
