...
public:
    ...
    void addBallGuides(threepp::BulletPhysics &bullet, threepp::Object3D &scene) const{
        ...
        BallGuideLeft->position.set(FlipperLeftPos.x - std::cos(threepp::math::PI / 6) * 4.25f * BallSize, FlipperLeftPos.y, FlipperLeftPos.z - std::sin(threepp::math::PI / 6) * 4.25f * BallSize + 22);
        BallGuideLeft2->position.set(FlipperLeftPos.x - std::cos(threepp::math::PI / 6) * 4 * BallSize - (std::cos(threepp::math::PI / 6) * 4 * BallSize) / 2, FlipperLeftPos.y, FlipperLeftPos.z - 3.5f * BallSize);

        BallGuideRight->rotateY(threepp::math::PI/6);
        BallGuideRight->position.set(FlipperRightPos.x + std::cos(threepp::math::PI / 6) * 4.25f * BallSize, FlipperRightPos.y, FlipperRightPos.z - std::sin(threepp::math::PI / 6) * 4.25f * BallSize + 22);
        BallGuideRight2->position.set(FlipperRightPos.x + std::cos(threepp::math::PI / 6) * 4 * BallSize + (std::cos(threepp::math::PI / 6) * 4 * BallSize) / 2, FlipperRightPos.y, FlipperRightPos.z - 3.5f * BallSize);
